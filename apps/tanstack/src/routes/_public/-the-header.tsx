import { headerStore, selectStainStyle, setActive, setHovered } from "@niama/ui/lib/stores/header";
...
export function TheHeader({ navs }: Pick<ReadPublicLayoutProps, "navs">) {
	...
	const stainStyle = useStore(headerStore, selectStainStyle);

	useEffect(() => setActive(navs.find((nav) => nav.hash === hash)?.hash), [hash, navs]);
		...
		<>
			...
			<Section className={THE_HEADER.menu()} id="top-2">
				<nav className={THE_HEADER.nav()} onMouseLeave={clearHovered}>
					<div aria-hidden="true" className={THE_HEADER.stain()} style={stainStyle}></div>
					{navs.map((nav) => (
...
function HeaderNav({ nav: { hash, text } }: HeaderNavProps) {
	...
	return (
		<button ref={ref} className={THE_HEADER.navLink()} type="button" onMouseEnter={handleOnMouseEnter}>
