...
export const themeStore = createStore<AppTheme>(getInitialTheme());
...
export const setStoredTheme = (theme: UserTheme): UserTheme => {
  ...
  try {
    ...
    // Ignore storage write failures when persistence is unavailable.
  }
...
export const toggleThemeWithTransition = (trigger: HTMLElement | null) => {
  ...
  if (prefersReducedMotion() || typeof document.startViewTransition !== "function" || !trigger) return applyTheme(nextTheme);
