...
export const SELECT = tv({
	slots: {
		content: `data-[side=bottom]:slide-in-from-top-2 data-[side=inline-end]:slide-in-from-left-2 
    ...
    ring-foreground/5 duration-100 data-[align-trigger=true]:animate-none data-closed:animate-out data-open:animate-in`,
		...
		item: `relative flex w-full cursor-default select-none items-center gap-2.5 rounded-xl py-2 pr-8 pl-3 text-sm 
    ...
    [&_svg]:pointer-events-none [&_svg]:shrink-0 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2`,
		...
		itemText: "flex flex-1 shrink-0 gap-2 whitespace-nowrap",
		label: "px-3 py-2.5 text-muted-foreground text-xs",
		scrollDownButton: `bottom-0 z-10 flex w-full cursor-default items-center justify-center bg-popover py-1 
    [&_svg:not([class*='size-'])]:size-4`,
		scrollUpButton: `top-0 z-10 flex w-full cursor-default items-center justify-center bg-popover py-1 
    [&_svg:not([class*='size-'])]:size-4`,
		separator: "pointer-events-none -mx-1 my-1 h-px bg-border/50",
		trigger: `flex w-fit items-center justify-between gap-1.5 whitespace-nowrap rounded-4xl border border-input 
    ...
    *:data-[slot=select-value]:gap-1.5 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 
    dark:hover:bg-input/50 [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0`,
		triggerIcon: "icon-[tabler--selector] pointer-events-none size-4 text-muted-foreground",
...
export function SelectGroup({ className, ...props }: SelectGroupProps) {
	return <SelectPrimitive.Group className={SELECT.group({ className })} data-slot="select-group" {...props} />;
}
...
export function SelectItem({ className, children, ...props }: SelectItemProps) {
	return (
		<SelectPrimitive.Item className={SELECT.item({ className })} data-slot="select-item" {...props}>
...
export function SelectLabel({ className, ...props }: SelectLabelProps) {
	return <SelectPrimitive.GroupLabel className={SELECT.label({ className })} data-slot="select-label" {...props} />;
}
...
export function SelectScrollDownButton({ className, ...props }: SelectScrollDownButtonProps) {
	return (
		<SelectPrimitive.ScrollDownArrow className={SELECT.scrollDownButton({ className })} data-slot="select-scroll-down-button" {...props}>
...
export function SelectScrollUpButton({ className, ...props }: SelectScrollUpButtonProps) {
	return (
		<SelectPrimitive.ScrollUpArrow className={SELECT.scrollUpButton({ className })} data-slot="select-scroll-up-button" {...props}>
...
export function SelectSeparator({ className, ...props }: SelectSeparatorProps) {
	return <SelectPrimitive.Separator className={SELECT.separator({ className })} data-slot="select-separator" {...props} />;
}
...
export function SelectTrigger({ className, size = "default", children, ...props }: SelectTriggerProps) {
	return (
		<SelectPrimitive.Trigger className={SELECT.trigger({ className })} data-size={size} data-slot="select-trigger" {...props}>
			{children}
...
export function SelectValue({ className, ...props }: SelectValueProps) {
	return <SelectPrimitive.Value className={SELECT.value({ className })} data-slot="select-value" {...props} />;
}
